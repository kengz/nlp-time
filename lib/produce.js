...
function getTimeChunk(syms) {
  var delimited = delimSyms(syms)
  var chunks = splitSyms(delimited, 'trinull')
  var candidates = orderChunks(chunks);
  // 4.
...
// production rule #4
// 1. use <bar> to split syms
// 2. if is <bar> <n> <bar>, default to <t:h>, with modding shit
// 3. recombine (without the bars)
// 4. run <T><T> ops
function barTTOps(syms) {
  // split them by bar
  var chunks = splitSyms(syms, 'bar');
  chunks = _.map(chunks, function(c, i) {
    // if chunk has only an 'n' symbol, make it into 'n-hour'
    if (_.size(c) == 1 && util.sName(c[0]) == 'n') {
...
// split the syms by the delimiter
// delimiter can be a plain string, or a symbol name (string)
function splitSyms(syms, delimiter) {
  // split the single array into matrix
  var matrix = [];
  var newRow = [];
  _.each(syms, function(s, i) {
    if (s == delimiter || util.sName(s) == delimiter) {
...
function opType(L, op, R) {
  ...
  if (opsName != 'r' || opsName != 'c') { opsName = '' };
...
// ehh maybe dont put down the bars yet before rT, so there can be tomorrow at 7-9 pm
