...
// recombine into norm string
// if token was normal form, has ISO normal forms, parse into ISO n return
// if is n: return value
// all else return org token
function normStr(syms) {
  var tokens = [];
...
var r = norm('05 October 2011 14:48 UTC 08/11 2020 tonight 3 hours and half after 9pm')
console.log(r)
  // if is number, return value, else return token
...
// compose(str)
// compose('tonight 3 hours after 9pm')
// yeah the mod n shits failing here, but let Matt take over for now
// compose('tonight at 3 hours after 9pm')
// compose('today at 3 hours after 9am')
// compose('2 days after tomorrow')
...
function atomicOp(Lval, op, Rval) {
  ...
  if (Lval == undefined) {
    ...
    if (defL && defR) {
      // if both are default, return left one
      return defL
    } else {
      // if one of them or both not default, just perform op without the default
      // make the into proper floats first
      l = parseFloat(Lval.toString().replace(/^=/, ''))
...
function TTOp(TL, op, TR) {
  ...
  // mutate into TL
  for (var k in TR.t) {
    TL.t[k] = atomicOp(TL.t[k], op, TR.t[k])
  }
  for (var k in TR.dt) {
