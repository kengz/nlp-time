...
function barTTOps(syms) {
  // split them by bar
  var chunks = util.splitSyms(syms, 'bar');
  ...
  // wait, need to respect bar ops. e.g. from/before/ago
  // combine back with the old bars
...
function nDefoTSyms(syms) {
  return _.map(syms, function(s) {
    return util.isSym(s, ['n']) ? nDefoT(s) : s;
  })
}
...
function reduce(syms, varArr, opArr) {
  ...
  _.each(syms, function(s, i) {
    if (!past || !s) {
      ...
      op = s;
      interOp = true;
      // the nDefoT for when past = 'n', s = 'bar'
...
// |n| -> |T|
syms = nDefoTSyms(syms)
// TT op
syms = reduce(syms, ['T'], ['bar'])
// syms = barTTOps(syms)
console.log('p#4', syms)
syms = reduce(syms, ['T'], ['r'])
console.log('p#5', syms)

// console.log(nDefoTSyms([symbol('at'), symbol('two'), symbol('at')]))
