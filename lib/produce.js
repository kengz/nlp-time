...
/**
 * [getTimeChunk description]
 * @param  {[type]} syms [description]
...
// return R
// 
// what if when you're missing on side
// binary op with default on either side, works by searching the op symbol
function optReduce(syms, varArr, opArr, Ldef, Rdef) {
  ...
  for (var i = 0; i < syms.length; i++) {
    var op = syms[i + 1],
      s = syms[i];
    // if next symbol is the op
    if (util.isSym(op, opArr)) {
      var L = (util.isSym(s, varArr)) ? s : Ldef;
      var R = (util.isSym(syms[i+2], varArr)) ? syms[i+2] : Rdef;
      
      if (util.isSym(s, varArr)) {
        L = s;
      } else {
        L = Ldef;
        res.push(s)
      }
      if (util.isSym(syms[i+2], varArr)) {
        // consume the right, skip in loop
        R = syms[i+2]
        i++;
      } else {
        R = Rdef;
      }
      res.push(execOp(L, op, R))
      // skip the op
      i++;
    } else {
      res.push(s)
...
// !TT op for t atomicOp is override, not adding except for default '='
function ToTOp(L, op, R) {
  var Ttype = ['t', 'dt']
  for (var i = 0; i < Ttype.length; i++) {
    var _Ttype = Ttype[i];
    var concatKeys = Object.keys(L[_Ttype]).concat(Object.keys(R[_Ttype]))
...
function lowestOverride(t) {
  ...
  var lowestOverable = _.find(util.timeUnitOrder.reverse(), function(unit) {
...
var str = "quarter pop pop pop four hour and half from 17 and half pm pop pop pop at 2st at 7 tonight"
...
function compose(str) {
  ...
  console.log('p#3', syms)
    // |n| -> |T|
  syms = nDefTSyms(syms)
    // add autohour-modding
    // TT op/o
  syms.pop()
  // syms = reduce(syms, ['T'], ['op', 'o'])
  syms = optReduce(syms, ['T'], ['op', 'o'], null, symbol(util.nowT()))
...
console.log(util.TtoStr(symbol(util.nowT())))
