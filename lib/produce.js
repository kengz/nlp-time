...
function nTnRedistribute(syms) {
  ...
  for (var i = 0; i < syms.length; i++) {
    var s = syms[i]
    if (util.sName(s) != 'n') { continue; };
    // 1.

    var R = syms[i+1]
    if (util.sName(R) == 'T') { continue; };
    // 2.2
    ...
    if (!syms[LLi] || util.sName(syms[LLi+1]) != 'T') { continue; };

    // 4.2
    syms.splice(LLi, 0, carry)
    syms = _.flatten(syms)
  }
