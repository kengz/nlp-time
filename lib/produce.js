...
// exececute binary operation, note that order matters,
...
function chunkSyms(syms) {
  ...
  var matrix = [];
  var newRow = [];
  _.each(newSyms, function(s, i) {
    if (s == 'trinull' || i == newSyms.length - 1) {
...
// get the most reasonale chunk of syms to parse for time
function getTimeChunk(syms) {
  var chunks = chunkSyms(syms)
