...
var re = {
  // 12/20 - 12/21
  MMsDDdMMsDD: /^(?!.*\d{1,4}\/\d{1,4}\s*-\s*\d{1,4}\/\d{1,4}\/)\D*(\d{1,4})\/(\d{1,4})\s*-\s*(\d{1,4})\/(\d{1,4})/g,
  // 12/22 - 23
  MMsDDdDD: /^(?!.*\d{1,4}\/\d{1,4}\s*-\s*\d{1,4}\/)\D*(\d{1,4})\/(\d{1,4})\s*-\s*(\d{1,4})/g,
  // 12/24
  MMsDD: /^(?!.*\d{1,4}\/\d{1,4}\/)\D*(\d{1,4})\/(\d{1,4})/g,
  // 05:30pm, 0530pm, 1730, 1730pm, 1730[re:h], remove the [re:h]
  hhcmm: /^(?!.*\/\d{1,2}\:?\d{2})\D*(\d{1,2})\:?(\d{2})\s*(\S+)*/g,
}
...
// console.log("d2", d2)
  // fix conflict by not taking the matched regex in subnormal
  // var d2 = _.map(d1.strArr, parseSubnormal)
  // console.log('d2', d2)
