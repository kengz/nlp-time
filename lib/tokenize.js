...
function tokenize(str) {
  ...
  // 3. parse english forms
  // lemmatize tokens with lookforward of up to 2-gram
  var skip = false;
  var symbols = [];
  _.each(tokens, function(tok, i) {
    ...
    var oneGram = tok,
      ...
      oneLemma = util.lemma(oneGram),
      twoLemma = util.lemma(twoGram);
...
function tokenize(str) {
  var pStr = parseNormalSubnormal(str)
  var tStr = pStr.replace(/\s+(\d+)([a-zA-Z]+)/g, ' $1 $2').replace(/\s+/g, ' ')
  var tokens = tStr.split(' ');
  // 3. parse english forms
  var symbols = _.map(tokens, symbol)
  return {
    str: str,
    tokens: tokens,
    symbols: symbols
...
function parseSubnormal(str) {
  ...
  if (m = re.MMsDDdMMsDD.exec(str)) {
    ...
    var unit = _.includes(['t', 'dt', 'T'], util.sName(sym)) ? (m[3] || '') : '';
