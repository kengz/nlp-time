...
/**
 * The symbol constructor, given a string, lemmatize it, then return a symbol from {∅=null,op,c,r,n,t,dt,custom,f}.
 * i.e. str -> lemma(str) -> new <symbol-name>(symbol-value) -> return
 ...
 * // => dt { h: '1' }
 * symbol('tonight')
 * // a custom object containing <t>, <dt>
 * // => custom { t: t { h: '=9' }, dt: dt { h: '12' } }
 * symbol('unrecognized')
...
function t(value) {
  ...
  while (value) {
    // default number is "1"
    var number = _.first(value.match(/^\d+/)) || "1";
...
/**
 * The custom constructor, implementation-specific, is a linear combination of <t> and <dt>.
 * Used to capture the human customs, e.g. noon, afternoon, dawn, evening, today, tonight, Sunday, fortnight, weekdays, weekends, christmas, spring, summer, holidays etc.
 * To specify custom in maps.json, follow the syntax:
 ...
 * @param  {string} value from the Symbol.
 * @example
 * var cus = new custom("t:=7h,dt:0h")
 * // => custom { t: t { h: '=7' }, dt: dt { h: '0' } }
 * cus.t
 * // => t { h: '=7' }
 * cus.dt
 * // => t { h: '0' }
